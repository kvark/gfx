...
use super::{
    ...
    Shot, Level, Primary, Secondary, DescriptorSetOffset,
};
...
pub struct RenderPassInlineEncoder<'a, B: Backend, L: Level>(RenderSubpassCommon<B, &'a mut B::CommandBuffer>, PhantomData<L>)
...
impl<'a, B: Backend, L: Level> RenderPassInlineEncoder<'a, B, L> {
        ...
        T::Item: Borrow<ClearValue>,
    {
        ...
        RenderPassInlineEncoder(
            RenderSubpassCommon::new(&mut cmd_buffer.raw),
            PhantomData,
        )
    }
...
impl<'a, B: Backend> RenderPassSecondaryEncoder<'a, B> {
    ...
    pub fn execute_commands<I, P: Shot>(&mut self, cmd_buffers: I)
    where
        I: IntoIterator<Item = CommandBuffer<B, Subpass, P, Secondary>>,
    {
        self.0.execute_commands(cmd_buffers);
    }

    /// Starts a new subpass with inline commands.
    pub fn next_subpass_inline(self) -> RenderPassInlineEncoder<'a, B, Primary> {
        self.0.next_subpass(SubpassContents::Inline);
        RenderPassInlineEncoder(RenderSubpassCommon::new(self.0), PhantomData)
    }
